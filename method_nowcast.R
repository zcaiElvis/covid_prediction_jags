...
model.loc <- ("jags_models/nowcast.txt")
jagsscript <- cat("
model {  
...
jags.data <- list("Y" = covid_nc$reported, "X"=covid_nc$revised, "N" = N)
# jags.params <- c("q", "r", "u")
mod_ss <- jags.model(file = model.loc, data=jags.data)
run_jag <- coda.samples(mod_ss, variable.names = c("u", "q", "r", "X[779]", "X[778]"), n.iter=100000)
